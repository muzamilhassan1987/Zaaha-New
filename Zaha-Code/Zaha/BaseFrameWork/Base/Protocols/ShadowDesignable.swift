...
public extension ShadowDesignable where Self: UIView {
    ...
    fileprivate func addDropShadow() {
        ...
        let path:UIBezierPath = UIBezierPath.init(rect: CGRect.init(x: 0, y: self.frame.height, width: hasBaseWidthConstraint == true ? DesignUtility.convertToRatio(self.bounds.width) : self.bounds.width, height: 10))
